...
public class GestureScript : MonoBehaviour {
	...
	void GamePlay(){
		...
		if (checkResult == true) {
			if (gestureResult.GestureClass == randomGesture [0].name && gestureResult.Score > 0.8) {
				colorArray [0] = 1;
				gestureResult.GestureClass = "";
				batAnimator.SetTrigger ("Attack");
				SoundManagerScript.instance.playRandom (3, batClip);
			} else if (gestureResult.GestureClass == randomGesture [1].name && gestureResult.Score > 0.8) {
				colorArray [1] = 1;
				gestureResult.GestureClass = "";
				goatAnimator.SetTrigger ("Attack");
				SoundManagerScript.instance.playRandom (4, sheepClip);
			} else if (gestureResult.GestureClass == randomGesture [2].name && gestureResult.Score > 0.8) {
				colorArray [2] = 1;
				gestureResult.GestureClass = "";
